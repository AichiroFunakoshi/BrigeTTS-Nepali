...
2. 開発用 Node.js を Node 22 に切り替え:
   ...
   Playwright 1.60.0 は Node 24/25 環境で起動が不安定になることがあるため、このプロジェクトでは Node 22 を前提にします。`nvm` を使う場合は `.nvmrc` に従って `nvm use` を実行してください。

3. 依存関係と Playwright Chromium を準備:
...
5. ローカルのHTTPSサーバーを起動（マイク アクセスに必要）:

   Python 3.14以降を使用:
   ```bash
   python3 -m http.server 8443 --tls-cert cert.pem --tls-key key.pem
   ```

   Python 3.13以前の `http.server` CLI はHTTPSオプションを持たないため、VS CodeのLive Server with SSLなどを使用してください。

6. モバイルブラウザでローカルサーバーのHTTPS URLを開く

7. 初回使用時にOpenAI APIキーを入力

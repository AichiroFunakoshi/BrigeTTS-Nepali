...
> [!IMPORTANT]
> **ネパール語TTSの現状（2026-09-07）**: iPhone実機では、OS内蔵のネパール語音声による読み上げが動作しないことを確認しています。Appleが公開する[VoiceOver対応言語一覧](https://support.apple.com/en-mide/111748)にもネパール語は含まれていません。これはアプリの翻訳機能の不具合ではなく、iPhone標準の音声対応範囲によるものです。設定の「日本語の音声読み上げ（TTS）」は残し、ネパール語→日本語の翻訳結果だけを読み上げます。日本語→ネパール語の翻訳結果は表示のみです。ネパール語読み上げには将来、外部の音声合成サービスとの連携が必要です（提供時期は未定）。
>
...
- **日本語TTS（音声読み上げ）**: ネパール語→日本語の翻訳結果を、翻訳ボックスのタップで再生/停止。ON/OFF・速度・日本語音声の設定を利用可能。日本語→ネパール語の翻訳結果は、iPhone標準のネパール語音声に対応していないため読み上げません
- **会話モード（オプション・日本語TTS）**: 音声入力の停止後、ネパール語→日本語の最後の翻訳結果を自動で読み上げる機能（デフォルトOFF）
...
- 対応言語ペアは日本語⇄ネパール語のみ
- iPhone実機ではOS内蔵機能によるネパール語TTSが動作しない。ネパール語→日本語の翻訳結果は日本語TTSで読み上げ可能で、日本語→ネパール語の翻訳結果は表示のみ。ネパール語読み上げには将来、外部音声合成サービスとの連携が必要（対応方法と時期は未定）

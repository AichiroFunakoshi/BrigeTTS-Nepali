...
- **最新リリース**: **v2.8.0**（配信済み）= 初回セットアップガイドF14。F13（言語自動判定）は見送り確定（2ボタン開始が言語宣言を兼ねるため）。v2.7.1=更新通知バナー＋配信手順書、v2.7.0=レイテンシ計測F12＋接続ウォームアップ。それ以前: v2.6.1=プロンプト品質修正（評価50ケース**100%**・致命的誤訳0件=KPI-3達成）、v2.6.0=設定エクスポート・インポート/評価ハーネス、v2.5.0=翻訳モード/ユーザー辞書/デバウンス自動適用
- **未リリースの改良**: なし
...
- **次にやること**: ①実機レイテンシ実測値の収集（設定画面→ユーザー報告待ち）→増分翻訳(設計§6順4)の要否判断 ②F15（APIキー運用の安全化）の調査 ③同僚テスト（KPI-5）の準備。ロードマップ残: v2.8完了条件=初見の同僚が補助なしでセットアップ完了

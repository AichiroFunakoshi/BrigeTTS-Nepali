...
- **最終更新**: 2026-07-08
- **作業ブランチ**: `main`（作業ブランチはすべてマージ済み・削除。順送りβは **PR #64 マージ済み**）
...
- **未リリースの改良**: **翻訳方式「順送り（β）」**（PR #64 マージ済み・タグ未発行。設定で切替・既定は従来方式で挙動不変。リリース＝`v*`タグはA/B比較の結果を見てから）。POST_MEASUREMENT_PLAN §3の増分翻訳（凍結方式）＋停止時整形パス（順送りの語順骨格を保持して誤訳・冗長を修正。根拠: 児島ら NLP2026 C8-15）。※実測ゲート（§1）はユーザー判断でスキップし、**標準方式との直接A/B比較用**として先行実装した

...
- **最終更新**: 2026-07-08（同日2回目: 実機フィードバック対応）
...
- **最新リリース**: **v2.9.2**（配信済み・2026-07-08）= 実機フィードバック対応（PR #74。辞書「誤認識される表記」/TTS頭切れ修正/日本語読み上げ音声選択/異常終了検出）。それ以前: v2.9.1=**翻訳方式「順送り（β）」**（PR #64。設定で切替・既定は従来方式で挙動不変・標準方式との直接A/B比較用。※POST_MEASUREMENT_PLAN §1の実測ゲートはユーザー判断でスキップして先行実装）＋仕様書/**石畳方式**文書（PR #66）。それ以前: v2.9.0=レイテンシ計測Issue送信＋API使用量可視化F15、v2.6.1=プロンプト品質修正、v2.6.0=設定エクスポート/評価ハーネス、v2.5.0=翻訳モード/ユーザー辞書/デバウンス自動適用
- **未リリースの改良**: なし
- **順送りβの設計文書**: 仕様書=[MONOTONIC_BETA_SPEC.md](MONOTONIC_BETA_SPEC.md)、設計思想（論文形式・参考文献付き・他システム流用チェックリスト）=[ISHIDATAMI_SCHEME.md](ISHIDATAMI_SCHEME.md)（**石畳方式**と命名）
...
- **次にやること**: **ユーザー側**=①本PRマージ・リリース後、実機確認（辞書に「表記=禄寿園、英語=Rokujuen、誤認識される表記=60円、六十円」を登録して検証。TTS頭切れ・日本語音声選択も）②フリーズ再発時は次回起動時に出るエラーレポートボタンから送信（#71の原因特定材料）③同僚テスト（POST_MEASUREMENT_PLAN §5）。**エージェント側**=①#73（順送り既定化・二段階移行）の実施判断をユーザーに確認 ②#72（暫定訳並走表示）の設計 ③異常終了レポートが届いたら#71の根本修正（Swift側）に着手

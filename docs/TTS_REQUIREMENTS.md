...
この文書の後続セクションは、TTS機能を実装した当初の要件と設計を記録したものです。現在の利用可否については、以下の実機確認結果を優先します。

- iPhone実機では、OS内蔵機能によるネパール語TTSが動作しないことを確認済み
- 設定の「日本語の音声読み上げ（TTS）」は、ネパール語→日本語の翻訳結果に利用できる
- TTSのコード、設定項目、会話モードとの連携は削除せず保持する
- 将来、外部の音声合成サービスなどと連携してネパール語TTSを提供するかは未定
- 日本語TTSを含む利用可能な音声は、OS、ブラウザ、端末へインストールされた音声に依存する

したがって、以下にある「ネパール語音声で読み上げ」「デフォルトON」などの記述は実装当初の要件であり、現在のiPhoneにおける動作保証を意味しません。

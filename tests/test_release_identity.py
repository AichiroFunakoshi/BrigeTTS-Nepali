...
class IndependentReleaseIdentityTests(unittest.TestCase):
    ...
    def test_current_release_version_and_artifact_names_are_consistent(self):
        ...
        self.assertIn('"version": "1.0.2"', package)
        self.assertIn('"version": "1.0.2"', package_lock)
        self.assertIn("BrigeTTS(Nepali) v1.0.2", index)
        self.assertIn("BrigeTTS(Nepali) v1.0.2", smoke)
        self.assertIn("brige-tts-nepali-v3", service_worker)
